...
class NoticePresenter: NoticePresenterProtocol {
    ...
    private func processNewsResponse(model:NewsResponse) -> [NotiItem] {
        ...
        model.newsList.forEach({ (item) in
            ...
            items.append(NotiItem(notiTitle: item.eDMSource, notiContent: item.eDMTitle , notiId: nil, notiDate: item.publishDate, unreadMark: item.unreadMark, linkType: item.linkType, linkValue: item.linkValue, apiNotiType: nil))
        })
        return items
    }
}
